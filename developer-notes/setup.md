...
One problem with the above procedure is that many static code analysis tools will not
be able to find the S3QL sources (see [pylance note](https://github.com/microsoft/pylance-release/blob/main/TROUBLESHOOTING.md#editable-install-modules-not-found), [meson-python bug](https://github.com/mesonbuild/meson-python/issues/555)).
...
sources, but not be able to find the compiled extension module. However, the latter
should not be a problem for static analysis.
